...
class Producer:
    def __init__(self, datatype):
        self.type = datatype
        self.queue = [middleware.XMLQueue(f"/tudo/bem/contigo/teste/de/topico/grande/a/ser/feito/atualmente/granda/drena", middleware.MiddlewareType.PRODUCER)]
        if datatype == "temp":
            self.gen = self._temp
        elif datatype == "msg":
            self.gen = self._msg
        elif datatype == "weather":
            self.queue = [middleware.XMLQueue(f"/{self.type}/temperature", middleware.MiddlewareType.PRODUCER),
                          middleware.XMLQueue(f"/{self.type}/humidity", middleware.MiddlewareType.PRODUCER), 
